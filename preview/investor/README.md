...
- Company overview: premiums versus net income / premiums; bubble area is proportional to average membership, with a diamond for MCS Life's missing membership. Select a point to inspect it. Sort nine insurers and compare up to three.
...
Treat this as an insurer research publication: a crisp light canvas for sustained comparison, a compact navy command bar, large tabular figures and one dark selected-company panel. Teal indicates positive results, rose indicates loss, and amber only marks the selected company or a documented source caveat. Meaning also appears in signs, labels and text. Give charts direct explanations and source access. Preserve the three-view hierarchy and identical annual controls; do not add decorative heroes, unrelated imagery, synthetic scores or simulated time series. The relevant question is which entity merits further diligence and why.

`design-tokens.css` owns reusable colors and surface values; `chart-theme.js` owns ECharts axis, grid, typography and tooltip defaults; `style.css` owns layout and responsive/print rules. Charts use SVG, run a single 550ms entry animation per chart identity, and respect reduced motion. Fixed portfolio axes are shared by both years; recheck data bounds when swapping the dataset.

## Validation

Verified unchanged source snapshots, all nine insurers, annual periods, proportional area encoding, member-month denominators, null handling for MCS Life, MMM page-level extraction, all coverage group/metric/period combinations and exact aggregate reconciliation. Tested unavailable quarterly-company states, comparison and source dialogs, sorting and SVG renders at narrow widths. Rendering, navigation and responsive screenshots are reviewed on the deployed preview before handoff.
