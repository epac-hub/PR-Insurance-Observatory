...
(function(){
  ...
  buildGlance=function(){
    ...
    var src=sourceLink(y==='25'?PLANILLA.sourceAnnual2025:PLANILLA.sourceAnnual2024,'OCS Planilla FY20'+y),both=sourceLink(PLANILLA.sourceAnnual2024,'FY2024')+' · '+sourceLink(PLANILLA.sourceAnnual2025,'FY2025');
    ...
    var cards=[['Market premiums',money(m.prem),y==='25'?'+'+((m.prem/prior.prem-1)*100).toFixed(1)+'% vs FY2024':'OCS Planilla · annual'],['Claims paid / premiums',(m.claims/m.prem*100).toFixed(1)+'%',y==='25'?'+'+((m.mlr-prior.mlr)*100).toFixed(1)+' pp vs FY2024':'Paid claims ÷ written premiums'],['Covered lives',m.lives.toLocaleString('en-US'),'OCS source-period count'],['Core 9 net income',money(ni),profitable+' of 9 entities profitable']];
    ...
    h+='<div class="obs-grid">'+panel('Premiums by coverage','FY20'+y+' · all 10 OCS coverage categories',coverageBars(y),src)+panel('How much goes to paid claims?','FY20'+y+' · OCS Planilla medical-expense market',claimsPanel(y),src+' · <button class="obs-detail-trigger" onclick="go(\'meth\')">View methodology</button>')+'</div>';
